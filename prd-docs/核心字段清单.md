...
| group_name | varchar(50) | 否 | 可选标签组；一期不强制分组，可为空（D1） |
...
### 选题版本 topic_version（二期）

> D8 已确认：一期不建此表；选题可人工修改但不保留版本历史，二期再评估。
...
| status | enum | ✅ | 草稿/待审核/已通过/已使用/已废弃；草稿为派生，已使用为人工标记的正式状态（D4） |
| reviewer_id | bigint | 条件 | 审核人 |
| reviewed_at | datetime | 条件 | 审核时间 |
| created_by | bigint | ✅ | 创建人 |
| created_at | datetime | ✅ | 创建时间 |
...
| id | bigint PK | ✅ | 主键 |
| name | varchar(100) | ✅ | 名称 |
| collection_method | enum | ✅ | 手动录入/CSV 导入；自动采集枚举仅作架构预留（D5） |
...
> D6：一期 `structured` 仅保存通用基础结构，不固化播放量、点赞、评论、涨粉等深度指标字段；深度指标二期迭代。
