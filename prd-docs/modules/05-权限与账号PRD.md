# 05-权限与账号 PRD

> 状态：✅ 初稿（基于 context 定稿产出）
> 模板：templates/prd-template.md
> 权威层级：context/ > templates/ > 本文件

## 1. 模块概述

权限与账号模块提供系统的访问控制基础：管理员（姐夫）与普通成员两级角色（6.7 确认），一期实现基础权限隔离（RBAC 预留），完整角色体系与操作审计二期。管理员可创建成员账号、分配角色与数据权限。

依赖关系：所有业务模块（资料/选题/脚本/分析）都受本模块权限控制。

## 2. 功能清单

| # | 功能 | In/Out | 关键规则 |
|---|---|---|---|
| 1 | 登录/登出 | ✅ In | 账号密码登录 |
| 2 | 账号管理 | ✅ In | 管理员创建/停用成员账号（6.7 确认） |
| 3 | 角色分配 | ✅ In | 管理员/成员两级 |
| 4 | 数据权限 | ✅ In（基础） | 管理员可配置成员可见数据范围 |
| 5 | 密码管理 | ✅ In | 修改密码/重置 |
| 6 | 完整 RBAC/操作审计 | Out（二期） | 主PRD §9 |

## 3. 交互流程

```mermaid
flowchart LR
    A[登录页] --> B[输入账号密码]
    B --> C{校验}
    C -->|成功| D[进入系统<br/>按角色加载权限]
    C -->|失败| A
    D --> E[管理员: 账号管理/角色/数据权限]
    D --> F[成员: 业务操作]
```

## 4. 关键规则

- 6.7 确认：先做权限隔离功能（RBAC 预留），后台管理员分配账号、配置权限和数据；
- 4.8 确认：脚本审核 MVP 仅自己（管理员），预留扩展；
- 一期两级角色：管理员（姐夫）/ 普通成员；
- 权限矩阵见 context/06 2.2（数据权限表格）；
- 完整 RBAC 角色体系与操作审计二期（主PRD §9）。

## 5. 状态机

```mermaid
stateDiagram-v2
    [*] --> 启用: 管理员创建
    启用 --> 停用: 管理员停用
    停用 --> 启用: 重新启用
    停用 --> [*]
```

## 6. 数据字段

引用 context/05，本模块涉及：

**用户**：id(主键)/用户名(唯一)/密码哈希(必填)/角色(管理员/成员)/状态(启用/停用)/创建人/创建时间/最后登录时间

**用户-角色-权限**（一期简化）：账号id/角色id/数据范围(全量/指定分类/指定数据源)

## 7. 权限

| 操作 | 管理员 | 成员 |
|---|---|---|
| 登录 | ✅ | ✅ |
| 创建/停用账号 | ✅ | ❌ |
| 分配角色/数据权限 | ✅ | ❌ |
| 修改自己密码 | ✅ | ✅ |
| 重置他人密码 | ✅ | ❌ |

## 8. 验收标准

- [ ] 管理员登录成功，看到账号管理入口；
- [ ] 成员登录成功，看不到账号管理入口（权限隔离生效）；
- [ ] 管理员创建新成员账号 → 成员可用新账号登录；
- [ ] 管理员停用账号 → 该账号无法再登录；
- [ ] 管理员给成员分配"仅指定分类资料"的数据范围 → 成员只能看到该分类资料；
- [ ] 成员修改自己密码成功，旧密码失效；
- [ ] 未登录访问业务页面 → 跳转登录；
- [ ] 边界：账号密码错误 5 次 → 提示锁定（或限流，防爆破）；
- [ ] 边界：管理员不能删除自己（防锁死）。

## 9. 待确认事项

- 登录方式是否要支持密码之外的（企业微信/验证码）→ 二期；
- 成员数量上限一期不做限制（预计个位数）。